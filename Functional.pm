...
use Carp;
no strict 'refs';
use vars qw($VERSION @ISA @EXPORT @EXPORT_OK $INFINITE);
require Exporter;
...
# I know, I know, it shouldn't *really* export everything

@EXPORT = qw(show inc double square cons max min even odd 
...
Increases the value passed by 1. eg: inc(2) = 3.
In Haskell:
...
Doubles the passed value. eg: double(3) = 6.
In Haskell:
...
numbers. eg: gcd(144, 1024) = 16. In Haskell:
...
eg: lcm(144, 1024) = 9216. In Haskell:
...
eg: id([1..6]) = [1, 2, 3, 4, 5, 6]. In Haskell:
...
Returns the first argument of 2 arguments.
eg: const(4, 5) = 4. In Haskell:
...
distinct from the Perl 'last' command. eg: Last([1..6]) = 6. In Haskell:
...
eg: tail([1..6]) = [2, 3, 4, 5, 6]. In Haskell:
...
eg: init([1..6]) = [1, 2, 3, 4, 5]. In Haskell:
...
infinite lists. eg: Map("double", [1..6]) = [2, 4, 6, 8, 10, 12].
In Haskell:
...
finite lists! eg: Length([1..6]) = 6. In Haskell:
...
Applies function f to the pairs (z, xs[0]),
(f(z, xs[0], xs[1]), (f(f(z, xs[0], xs[1])), xs[2]) and
...
of 1..6: foldl(sub { $_[0] + $_[1] }, 0, [1..6]) = 21.
In Haskell:
...
of 1..6: foldl1(sub { $_[0] + $_[1] }, [1..6]) = 21.
In Haskell:
...
ie returns the list z, f(z, xs[0]), f(f(z, xs[0]), xs[1]), f(f(f(z, xs[0]), xs[1]), xs[2])
and so on.
eg: scanl(sub { $_[0] + $_[1] }, 0, [1..6]) = [0, 1, 3, 6, 10, 15, 21].
In Haskell:
...
This is a variant of scanl where the first value of xs is taken
as q. Returns a list of all the intermedia values that foldl would compute.
ie returns the list f(xs[0], xs[1]), f(f(xs[0], xs[1]), xs[2]), f(f(f(xs[0], xs[1]), xs[2]), xs[3])
and so on.
eg: scanl1(sub { $_[0] + $_[1] }, [1..6]) = [1, 3, 6, 10, 15, 21].
In Haskell:
...
This is similar to foldl but is folding from the
right instead of the left.
Note that foldr should not be done to infinite lists.
eg: the following returns the sum
of 1..6: foldl(sub { $_[0] + $_[1] }, 0, [1..6]) = 21.
In Haskell:
...
This is similar to foldr1 but is folding from the
right instead of the left. Note that foldr1 should
not be done on infinite lists.
eg: foldr1(sub { $_[0] + $_[1] }, [1..6]) = 21.
In Haskell:
...
scanr(sub { $_[0] + $_[1] }, 0, [1..6]) = [0, 6, 11, 15, 18, 20, 21].
In Haskell:
...
not be done on infinite lists. eg:
scanr1(sub { $_[0] + $_[1] }, [1..6]) = [6, 11, 15, 18, 20, 21].
In Haskell:
...
This returns the infinite list (x, f(x), 
f(f(x)), f(f(f(x)))...) and so on. eg:
...
elements are x. eg: take(4, repeat(42)) = [42, 42, 42, 42].
In Haskell:
...
Returns a list containing n times the element x.
eg: replicate(5, 1) = [1, 1, 1, 1, 1]. In Haskell:
...
Returns a list containing the first n elements from the list xs.
eg: take(2, [1..6]) = [1, 2]. In Haskell:
...
Returns a list containing xs with the first n
elements missing. eg: drop(2, [1..6]) = [3, 4, 5, 6].
In Haskell:
...
Splits the list xs into two lists at element
n. eg: splitAt(2, [1..6]) = [[1, 2], [3, 4, 5, 6]].
In Haskell:
...
takeWhile(sub { $_[0] <= 4 }, [1..6]) = [1, 2, 3, 4].
In Haskell:
...
true. Returns the list. eg: dropWhile(sub { $_[0] <= 4 }, [1..6]) = [5, 6].
In Haskell:
...
Splits xs into two lists, the first containing the
first few elements for which p(that element) is 
true. eg: span(sub { $_[0] <= 4 }, [1..6]) = [[1, 2, 3, 4], [5, 6]].
In Haskell:
...
Splits xs into two lists, the first containing the
first few elements for which p(that element) is
false. eg: break(sub { $_[0] >= 4 }, [1..6]) = [[1, 2, 3], [4, 5, 6]].
In Haskell:
...
Breaks the string s into multiple strings, split
at line boundaries. eg: lines("A\nB\nC") = ['A', 'B', 'C'].
In Haskell:
...
whitespace boundaries. eg: words("hey how random") = ['hey', 'how', 'random'].
In Haskell:
...
strings into one, joined by newlines. eg: unlines(['A', 'B', 'C']) = "A\nB\nC";
In Haskell:
...
strings into one, joined by a space. eg: unwords(["hey","how","random"]) = 'hey how random'.
In Haskell:
...
order. Note the capital R, so as not to clash with the Perl command 'reverse'.
You should not try to Reverse an infinite list.
eg: Reverse([1..6]) = [6, 5, 4, 3, 2, 1]. In Haskell:
...
the Perl command 'and'. You should not try to And an infinite
...
Returns true if x is not present in x. You should
not do this with infinite lists. Note that this assumes
that x and xs are numbers.
...
sub notElem {
  my($x, $xs) = @_;
  return all(sub { $_[0] != $x }, $xs);
}


=item lookup key xys

This returns the value of the key in xys, where
xys is a list of key, value pairs. It returns undef
if the key was not found. You should not do 
this with infinite lists. Note that this assumes
that the keys are strings. eg: lookup(3, [1..6]) = 4.
In Haskell:
...
sub TIEARRAY {
  my $class = shift;
  my $closure = shift;
  my $size = shift || $Functional::INFINITE;
...
sub FETCH {
  ...
  if ($idx == $Functional::INFINITE or $idx == $Functional::INFINITE-1) {
